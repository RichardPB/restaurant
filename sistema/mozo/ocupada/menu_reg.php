<?php
	...
	$cnn = conectar();
         ...
         $result1 = mysql_query($query1, $cnn) or die("Error de conexion1");
         $cantidad1 = is_resource($result1) ? mysql_num_rows($result1) : 0;
        
        $query2 = "SELECT * FROM `v_mesa` WHERE `idmesa`='".$_GET['q']."'";
        $result2 = mysql_query($query2, $cnn) or die("Error de conexion2");
         $fil1 = mysql_fetch_array($result2);

               $r=date('Y-m-d');
              
          $query3 = "select  * FROM pedido where mesa_id='".$_GET['q']."' and fechapedido='$r'";
          $result3 = mysql_query($query3, $cnn) or die("Error de conexion3");
          $fil3 = mysql_fetch_array($result3);
       
          $query4 = "SELECT `id`,plato.Nombre,cantidad,pedido_subtotal FROM `dt_pedido`INNER JOIN carta on carta.carta_id=dt_pedido.IdCarta INNER JOIN plato on plato.Id_Plato=carta.plato_id where idpedido='$fil3[0]'";
          $result4 = mysql_query($query4, $cnn) or die("Error de conexion3");
           $query5 = "SELECT `id`, bebidas.Nombre,cantidad,pedido_subtotal FROM `dt_pedido` INNER JOIN carta on carta.carta_id=dt_pedido.IdCarta INNER JOIN bebidas on bebidas.Id_Bebidas=carta.bebida_id where idpedido='$fil3[0]'";
           $result5 = mysql_query($query5, $cnn) or die("Error de conexion3");
       
           $query6 = "SELECT `id`,plato.Nombre,cantidad,pedido_subtotal FROM `dt_pedido`INNER JOIN carta on carta.carta_id=dt_pedido.IdCarta INNER JOIN plato on plato.Id_Plato=carta.plato_id where idpedido='$fil3[0]'";
          $result6 = mysql_query($query6, $cnn) or die("Error de conexion3");
       
          $query7 = "SELECT `id`, bebidas.Nombre,cantidad,pedido_subtotal FROM `dt_pedido` INNER JOIN carta on carta.carta_id=dt_pedido.IdCarta INNER JOIN bebidas on bebidas.Id_Bebidas=carta.bebida_id where idpedido='$fil3[0]'";
           $result7 = mysql_query($query7, $cnn) or die("Error de conexion3");
?>
...
<form id="form_pedido_reg" name="form_pedido_reg" class="form-vertical" >
	...
	<div class="panel-body">
		    ...
		    <span class="input-group-addon"><span class="glyphicon glyphicon-calendar" aria-hidden="true">&nbsp;Fecha&nbsp</span></span>
                    <input type="date"  name="txtfecha" id="txtfecha" class="form-control"  value="<?php echo $fil3[1];?>">
		    </div>
                    <br>
                    <div class="input-group">
